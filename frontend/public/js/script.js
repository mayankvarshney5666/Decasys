...
document.addEventListener("readystatechange", (event) => {
    const readyState = "complete";
    if (document.readyState == readyState) {
        const loaderEl = document.querySelector(".fullpage-loader"); // Select the loader element

        if (loaderEl) { // Ensure that loaderEl is not null
            loaderEl.classList.add("fullpage-loader--invisible"); // Add the CSS class
            setTimeout(() => {
                loaderEl.parentNode.removeChild(loaderEl); // Remove the loader element from its parent after 100ms
            }, 100);
        }
    }
});
