...
export default function Subcategorylist() {
  ...
  const getAllProductBySubCategory = async () => {
    const resource = await fetch(
      `${apiUrl}/getAllProductBySubCategory/${_id.id}`
    );
    const result = await resource.json();
  ...
  useEffect(() => {
    ...
    getbrand();
    // getProduct()
    getAllProductBySubCategory(_id?.id)
                            ...
                            if (!loopExecuted) {
                              ...
                              return (
                                <React.Fragment key={index}>
                                  ...
                                  <h5 className="price"><span>₹  {item?.price}</span></h5>
                                </React.Fragment>
                              );
                            }
